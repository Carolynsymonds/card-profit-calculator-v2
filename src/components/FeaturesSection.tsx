      ...
      <div className="mx-auto text-center space-y-12">
        ...
        <div className="grid md:grid-cols-3 gap-8 pt-8 max-w-[900px] self-center m-auto">
          {siteContent.features.items.map((feature, index) => {
            // Check if icon is a URL (SVG), local image, or lucide icon name
            const isUrl = feature.icon.startsWith('http');
            const isLocalImage = feature.icon.startsWith('/');
            const isSvg = feature.icon.includes('.svg');
